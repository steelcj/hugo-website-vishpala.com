...
With Pagefind on `PATH`, the build script at `scripts/build.sh` in any Hugo project can call it directly after `hugo --minify`:
...
Move back into the site's parent directory and start the Hugo development server:
...
|---------|--------|-------|
| 0.1.3 | Draft | Corrected repo owner to Pagefind throughout; corrected section 8 dev server description; corrected References |
